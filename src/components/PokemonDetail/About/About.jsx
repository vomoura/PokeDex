...
const REGIONAL_FORM_VERSIONS = { "legends-arceus": "hisui" };
...
export default function About({ pokemon, species, t }) {
  const [versionIdx, setVersionIdx] = useState(0);
  const [translatedFlavor, setTranslatedFlavor] = useState("");
  ...
  const rawFlavor = (versionIdx === 0
    ? v1?.flavor_text
    : v2?.flavor_text ?? v1?.flavor_text
  )?.replace(/\f/g, " ") || "";

  const needsTranslation = t.apiLang !== "en" && langEntries.length === 0;
  const isTranslating = needsTranslation && !translatedFlavor;

  useEffect(() => {
    if (!needsTranslation || !rawFlavor) {
      setTranslatedFlavor(rawFlavor);
      return;
    }
    setTranslatedFlavor("");
    translateText(rawFlavor, "pt").then(setTranslatedFlavor);
  }, [rawFlavor, needsTranslation]);

  const flavor = needsTranslation ? translatedFlavor : rawFlavor;
  const speciesName =
  ...
  return (
    <div className="tab-content">
      ...
      {isTranslating
        ? <p className="flavor-loading">Traduzindo...</p>
        ...
        <tbody>
          ...
          <tr>
            ...
            <td>
              {species?.egg_groups
                ?.map((g) => {
                  if (g.name === "no-eggs") return t.eggGroupNoEggs;
                  return g.name.replace(/-/g, " ").replace(/\b\w/g, (c) => c.toUpperCase());
                })
                .join(", ") || "—"}
            </td>
          </tr>
